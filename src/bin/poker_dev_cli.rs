...
fn main() {
    println!("poker_dev_cli: стартуем dev-CLI для покерного движка…");
    ...
    println!("[CLI] Создан основной стол id={} с тремя игроками.", main_table_id);
    ...
    // 6. Дополнительно: создаём несколько столов и запускаем на каждом одну тестовую раздачу
    println!();
    println!("================ MULTI-TABLE SIMULATION =================");

    let mut extra_table_ids: Vec<TableId> = Vec::new();

    // Создадим, например, 3 стола с разным количеством игроков (4–6).
    for n in 0..3 {
    ...
    for table_id in extra_table_ids {
        println!();
        println!("------ AUTO TABLE id={} | Single SimpleCheckCall hand ------", table_id);
...
fn play_hand(
    ...
    println!("[CLI] Запускаем start_hand для table_id={}, hand_id={}.", table_id, hand_id);
    ...
    match manager.start_hand(table_id, rng, hand_id) {
        Ok(()) => {
            ...
            println!("[CLI] start_hand успешно отработал. Дилер на столе {} = Some({}).", table_id, dealer);
            debug_print_table_state(manager, table_id);
        }
        Err(e) => {
            println!("[CLI] ОШИБКА в start_hand для стола {}: {:?}", table_id, e);
            debug_print_table_state(manager, table_id);
    ...
    if let Err(e) = run_scenario(manager, table_id, scenario) {
        println!("[CLI] ОШИБКА в run_scenario для стола {}: {:?}", table_id, e);
    }
...
fn run_scenario(
    ...
    loop {
        ...
        match manager.apply_action(table_id, action) {
            Err(e) => {
                println!("[CLI] ОШИБКА в apply_action на столе {}: {:?}", table_id, e);
                debug_print_table_state(manager, table_id);
                return Err(e);
            }
...
fn pick_base_action(
    ...
    if to_call_amount == 0 {
        ...
        match table.street {
            ...
            // На постфлопе иногда ставим минимальный bet (1 BB),
            // чтобы было движение и тест Bet/raise.
            ...
            Street::Showdown => PlayerActionKind::Check, // сюда по идее не попадём
        }
    } else {
        // Нужно доплатить
        if player.stack.0 <= to_call_amount {
            // Не хватает фишек — all-in.
...
fn pick_scenario_action(
    ...
    match scenario {
        Scenario::SimpleCheckCall => {
            // Полностью базовая стратегия.
            pick_base_action(table, engine, seat, player)
        }

        Scenario::WithFold => {
            // Цель: протестировать finish_hand_without_showdown.
            // Префлоп:
            //  step=1, seat=1 -> Fold
            //  step=2, seat=2 -> Fold
            // После этого остаётся только seat=0 → авто-победа без шоудауна.
        ...
        Scenario::WithRaises => {
            // Цель: протестировать Raise и min_raise.
            //
            // Префлоп:
            //  step=1, seat=2 -> Raise до 300 (с 100 до 300)
            // Остальные -> базовая логика (Call/AllIn).
            if table.street == Street::Preflop && step == 1 && seat == 2 {
                // Текущий bet = 100 (BB), min_raise = 100.
                // Raise до total_bet=300 → raise_size=200 ≥ min_raise.
        ...
        Scenario::WithAllInSidePots => {
            // Цель: протестировать all-in + side pots.
            //
            // Перед раздачей мы выставляем стеки:
            //  P1 (seat 0): 5000
            //  P2 (seat 1): 5000
            //  P3 (seat 2): 500
            //
            // Префлоп:
            //  step=1, seat=0 -> Raise до 1000
            //      (current_bet=100, raise_size=900, min_raise=100 → валидно)
            //  P2 и P3 по базовой логике:
            //      - у P2 to_call=900, stack>to_call → Call
            //      - у P3 to_call=900, stack<=to_call → AllIn (через базу)
            //
            // Contributions:
            //  seat0: 1000, seat1: 1000, seat2: 500
            // На шоудауне compute_side_pots построит основной и сайд-поты.
...
// чтобы видеть, как движок меняет board, пот, игроков, дилера и текущего актёра.
fn debug_print_table_state(manager: &TableManager, table_id: TableId) {
    ...
    // Выбираем "героя" — просто первого сидящего игрока.
    ...
    let dto: TableViewDto = build_table_view(
        table,
        engine_opt,
        |pid: PlayerId| format!("P{}", pid), // resolve_name
        |pid: PlayerId| pid == hero_id,      // is_hero
    );

    // Пот для вывода:
    // - если есть engine -> используем engine.pot.total (реальный текущий банк)
    // - иначе -> dto.total_pot (что вернул API-слой)
